...
if(isMobileAlert()){
    if($country == "Brazil") {
    echo ("<script> alert('ID da Sala Inválido') </script>");
    } else {
...
if(isMobile()){
    ...
    <main class="chat">
        ...
        if($country == "Brazil") {
            echo ("Erro: O ID da sala é inválido ou a sala não existe mais.");
        } else {
            echo ("Error: The room ID is invalid or the room doesn't exist anymore.");
        }
        ?>
        </h4>
