...
if(isMobileAlert()){
    if($country == "Brazil") {
    echo ("<script> alert('Desculpe, mas você foi banido dessa sala') </script>");
    } else {
    echo ("<script> alert('Sorry, but you have been banned from this room.') </script>");    
    }
}
?>
<!DOCTYPE html>
...
if(isMobile()){
    ...
    <main class="chat">
        ...
        if($country == "Brazil") {
            echo ('Erro: Desculpe, mas você foi banido dessa sala');
        } else {
            echo ("Error: I'm sorry, but you have been banned from this room");
        }
        ?>
        </h4>
            <h1 style="margin-top: 60px;"><?php
            if($country == "Brazil") {
